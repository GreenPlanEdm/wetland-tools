...
# Each block below names the references.csv source_id(s) that document its
# upstream citation. To resolve a citation, load data/references.rda and
# subset on source_id.

# AB Invasive Species Council (AISC) plant list — 4th ed. 2022
# Citation: aisc_2022 (see data/references.rda)
...
# Citation: goa_awcs_2015 (Appendix C + classification text); taxonomy from ITIS 2014.
...
# Citations: ab_wetland_plant_list_2021 (the AB list); usace_nwpl_2018 (upstream
# regional lists for WMVC / GP / NCNE / AK); lichvar_2012 (rating definitions).
...
# AB Rangeland Plants
# Citation: goa_range_plants_2023
...
# References / bibliography — package-wide
# Each row is a citation with a stable source_id slug. Other blocks in this
# script name source_id(s) in their header comment so any classification call
# can be traced from its data table → source_id → full citation. Three
# source_type families:
#   - agency_publication: government / agency lookup lists, technical docs,
#     and methodology guides (AWCS, AWIDD, WAIR, ANPC, USACE NWPL, etc.)
#   - regulation: statutory law (e.g. AB Weed Control Act)
#   - primary_literature: peer-reviewed papers (mostly the BD2025.042
#     salinity references; also Von Post 1922 and Newhall 1943)
#   - r_package: R packages used as data sources (e.g. aqp for Munsell)
# A few entries (AWIDD, WAIR, water-permanence guidance) still have TODO
# notes for verifying year and URL — see notes column.
references <- read.csv(
...
# Citations: beckingham_archibald_1996 (ecosite phases); goa_awcs_2015 (wetland
# class/form lookup target).
...
# Citations: ab_natural_regions_2006 (AB natural regions); usace_nwpl_2018
# (NWPL regional lists). Maps AB Natural Regions to USACE NWPL regions.
...
# Citation: von_post_1922 (original definition).
...
# Citations: aqp_pkg (R package providing the loaded values); newhall_1943
# (underlying renotation data published in the Journal of the OSA).
